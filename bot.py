...
def kisa_token(metin):
    ...
    if m:
        if m.group(2).lower() in SAYI_KELIME:
            return None
        try:
            mk = float(m.group(1).replace(",", ".")) if m.group(1) else 1.0
        except Exception:
            return None
        if mk <= 0 or mk > 1e12:
            return None
...
app.add_handler(MessageHandler(filters.ChatType.GROUPS & filters.UpdateType.MESSAGE & \~filters.StatusUpdate.ALL, kilit_kontrol), group=1)
